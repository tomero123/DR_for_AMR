...
if __name__ == '__main__':
    ...
    WINDOW_SIZE = 10 if len(sys.argv) <= 3 else int(sys.argv[3])
    ...
    trainer.run()
    print(f"Finished training for bacteria: {BACTERIA} model_folder_name: {model_folder_name} in {round((time.time() - now) / 3600, 4)} hours")
    now_date = datetime.datetime.now()
